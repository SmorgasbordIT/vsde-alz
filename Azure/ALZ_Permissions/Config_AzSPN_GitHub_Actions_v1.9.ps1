function Setup-AzureSpn {
    <#
        ...
        PS> . ./Config_AzSPN_GitHub_Actions_v1.9.ps1
    ...
    process {
        ...
        try {
            ...
            if (-not $azExistingSpn) {
                ...
                if ($azExistingSpn) {
                    ...
                    $startDate = Get-Date
                    $endDate = $startDate.AddDays($azSpnSecretExpiryInDays)
                    $secretObject = New-AzADAppCredential -ApplicationId $azSpnAppId -StartDate $startDate -EndDate $endDate
        ...
        try {
            ...
            $azSecrets = @{
                "AZ_CLIENT_ID"       = $azSpnAppId
                "AZ_CLIENT_SECRET"   = $azSpnSecret
                "AZ_SUBSCRIPTION_ID" = $azSubscriptionId
                "AZ_TENANT_ID"       = (Get-AzContext).Tenant.Id
            }
            ...
            foreach ($repo in $ghRepoNames) {
                foreach ($env in $ghEnvNames) {
                    foreach ($azSecretName in $azSecrets.Keys) {
                        ...
                        Write-Verbose "Setting secret '$azSecretName' in repo '$repo' for environment '$env'"
                        gh secret set $azSecretName --repo "$($ghOrgName)/$repo" --env $env --body $azSecretValue
                    }
                }
            }
            ...
            $azSpnSecret = $null
            $azSecrets["AZ_CLIENT_SECRET"] = $null
