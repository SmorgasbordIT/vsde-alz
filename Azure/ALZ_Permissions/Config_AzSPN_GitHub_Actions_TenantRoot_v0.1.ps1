    ...
    process {
        ...
        if (-not $azSpnSecretExpiryInDays) {
            ...
            switch -Regex ($azSpnDisplayNameUpper) {
                ...
                'STG|QA|UAT|PRE' { $azSpnSecretExpiryInDays = 180; break }
        ...
        try {
            ...
            if (-not $azExistingSpn) {
                ...
                $azTenantRootGroupId = (Get-AzManagementGroup).Name
                $azRootScope = "/providers/Microsoft.Management/managementGroups/$azTenantRootGroupId"

                # Create new SPN with role at tenant root scope
                Write-Verbose "Creating new Azure AD Service Principal at tenant root scope..."
                $azNewSpn = New-AzADServicePrincipal -DisplayName $azDisplayName -Role $azSpnRole

                Start-Sleep -Seconds 5

                # Assign Contributor Role to SPN at Tenant Root Group
                New-AzRoleAssignment -ApplicationId $azNewSpn.AppId `
